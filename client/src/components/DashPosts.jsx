...
const DashPosts = () => {
    ...
    const fetchPosts = async () => {
      try {
        const res = await fetch(
          `/api/v1/post/getposts?userId=${currentUser.userData._id}`
        );
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(
        `/api/v1/post/getposts?userId=${currentUser.userData._id}&startIndex=${startIndex}`
      );
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(
        `api/v1/post/deletepost/${postIdToDelete}/${currentUser.userData._id}`,
        {
          method: 'DELETE',
        }
      );
